...
| Multi-tenancy | This is a shared cloud service, not one deployment per clinic: a single backend serves many independent clinics ("tenants"), each fully isolated from the others' data. A tenant is created one of two ways: **self-serve signup** (a prospective admin establishes their own identity — password or Google — and fills in the clinic's profile themselves) or **`super_user`-provisioned invite** (a dedicated role that operates the service — provision/revoke tenants, service config, no access to any tenant's clinical data — fills in the clinic profile and an email address up front; the invited admin then picks password or Google and can edit that pre-filled profile before the account activates). Either way the tenant ends up with the same mandatory profile — name, mobile, clinic type (§4.1/§5.2/§5.4) |
...
erDiagram
    ...
    USERS {
        ...
        text password_hash "nullable — unset for a Google-only account, or before an invite is accepted, §5.4"
        text google_sub "nullable, unique — Google's stable subject ID; unset for a password-only account, §5.4"
        text full_name
...
- **`clinic_types` / `tenants.clinic_type_id`** — every tenant is a clinic of
  ...
  - **Set once, at provisioning** (§5.4) — picked alongside the rest of
    the mandatory clinic profile (name, mobile), either by the admin
    themselves during self-serve signup or by a `super_user` provisioning
    the tenant on their behalf. Changing a tenant's clinic type after the
    fact isn't a supported flow
...
CREATE TABLE users (
    ...
    password_hash     TEXT,                         -- nullable — unset for a Google-only account, or before an invite is accepted (§5.4)
    google_sub        TEXT UNIQUE,                  -- nullable — Google's stable subject ID; unset for a password-only account (§5.4)
    ...
    created_at        TEXT NOT NULL DEFAULT (datetime('now')),
    last_login_at     TEXT,
    CHECK (active = 0 OR password_hash IS NOT NULL OR google_sub IS NOT NULL)  -- an active account must have at least one way to authenticate (§5.4)
);
CREATE INDEX idx_users_tenant ON users(tenant_id);
-- No separate index for google_sub lookups (the callback's "does this
-- sub already have an account" check) — the UNIQUE constraint above
-- already creates one.
...
- **Provisioning: two paths, one shared completion step.** Both end up
  with the same mandatory clinic profile (name, mobile, clinic type — the
  three fields §5.2's `clinic_type_id` note references) and the same
  identity choice (password or Google, below) — the difference is only
  *who* fills in the clinic profile and *when* the tenant becomes usable:

  1. **Self-serve signup** — `POST /auth/signup` (§6, public). A
     prospective admin isn't invited by anyone: they establish their own
     identity (set a password, or authenticate with Google — below) and
     supply the clinic profile themselves in the same completion step. On
     success the tenant is created and the account is active immediately
     — there's no invite step, since nobody needs to vouch for who they
     are beyond the password they just set or Google's own verification.
  2. **`super_user`-provisioned invite** — `POST /platform/tenants` (§6,
     `super_user`-only). A `super_user` fills in the clinic profile **and**
     the prospective admin's email address, then provisions. This inserts
     the `tenants` row immediately (`status = 'active'`, `is_platform = 0`,
     `clinic_type_id` set, pre-populated with everything the `super_user`
     typed — including the tenant's name, still used as a placeholder for
     the first admin's `full_name` until there's a self-edit screen for
     it, a real gap worth a follow-up, not a fully solved edge) and a
     `users` row for the admin with `active = 0`, an `invite_token`/
     `invite_expires_at` set, and no `password_hash`/`google_sub` yet. An
     invite email goes out with a link carrying the token; the invite
     token is never returned in any API response body, only delivered by
     email. Clicking the link opens the accept-invite screen (§8.1), which
     fetches the tenant's already-entered clinic profile via
     `GET /auth/invite-preview?token=` (§6, public, read-only) and shows
     it back **pre-filled but editable** — the invited admin reviews it,
     corrects anything wrong or fills in what the `super_user` left blank,
     picks a password or Google to establish their own identity, and
     submits `POST /auth/accept-invite` (§6) with the token, their chosen
     credential, and the (possibly-edited) clinic profile. That call sets
     `password_hash`/`google_sub`, applies the final clinic profile to the
     tenant row, flips `active` to `1`, and clears the invite fields — the
     account lands signed in, same as a normal login.

  Either path also inserts a default `app_settings` row for the tenant
  (§5.1/§5.3), so `GET /settings` always has a row to return without the
  lazy-create special case `user_preferences` has.
- **Authentication methods: password and/or Google, not mutually
  exclusive by design.** `users.password_hash` and `users.google_sub`
  (§5.1/§5.3) are both nullable, and an account only needs one of them set
  to be active (enforced by a `CHECK` on the table, §5.3). Whoever's
  completing signup or accepting an invite picks exactly one at that
  moment:
  - **Password**: typed + repeated, validated client-side and re-validated
    server-side (≥8 characters, at least one letter and one digit — the
    same rule this local-storage build already enforces via
    `utils/password.ts`). `POST /auth/login` stays a plain email+password
    check, unaffected by any of this.
  - **Google**: `GET /auth/google` starts the OAuth 2.0 authorization-code
    flow — a redirect to Google's consent screen carrying a signed `state`
    param that encodes *why* (self-serve signup vs. accepting a specific
    invite token), so the callback knows what to do next without trusting
    anything the browser could tamper with. `GET /auth/google/callback`
    exchanges the returned code for tokens server-side — the client secret
    never reaches the browser — verifies the ID token's signature/issuer/
    audience against Google's public keys, and rejects `email_verified:
    false` outright, since this is the one place the app takes someone's
    word for their own identity without a password or invite token behind
    it. If the resulting `sub` (or email) already matches an existing
    `users` row, this is a login, not a signup: set the session cookie and
    redirect straight into the app. Otherwise the callback mints a
    short-lived, signed **signup ticket** — not a DB row, just a signed,
    expiring token carrying the verified email/`sub` and the decoded
    `state` — and redirects to the client's signup-completion or
    accept-invite screen with it; that screen's later `POST /auth/signup`
    or `POST /auth/accept-invite` call includes the ticket in place of a
    typed password, so the server never has to re-verify an identity it
    already checked.
  - **Linking the two on one account isn't implemented in this pass** — an
    account is created with whichever method was used at signup/invite-
    acceptance time and stays that way; there's no "add Google to my
    password account" (or vice versa) settings action yet. Flagged as a
    near-term follow-up in §13 — not blocked by anything here, since both
    columns are already independently nullable on the same row specifically
    so adding the second one later is a plain `UPDATE`, not a schema
    change.
...
- **Password strength** is enforced client-side (`utils/password.ts`:
  ...
  short passwords.
- **Google sign-in and the invite-prefill/edit flow above are spec-only,
  not implemented here** — this local-storage build has no real backend to
  hold an OAuth client secret or exchange an authorization code against,
  so signup/tenant-creation in this prototype stays exactly as described
  above: password-only, self-serve-only, no invite email, no
  `google_sub`/signup-ticket concept. `TenantForm` (superuser) already
  collects the full clinic profile via the same `ClinicProfileFields`
  `SignUpScreen` uses, though, so the "pre-fill the invited admin's clinic
  profile from what the superuser typed" behavior has no real gap left to
  close on the clinic-profile side once the invite flow itself gets built
  — only the identity/credential half (password vs. Google, the invite
  email, the ticket handoff) is genuinely new.
...
| `POST /auth/signup` | — (public) | Self-serve signup (§5.4) — a clinic profile plus a chosen credential: `{ password }` or `{ googleTicket }` (from `GET /auth/google/callback` below). Creates the tenant + an active `admin` user in one transaction and signs them in — no invite, no token | — |
| `GET /auth/google` | — (public) | Starts the Google OAuth flow — redirects to Google's consent screen with a signed `state` param encoding the caller's intent (`signup`, or `accept-invite` + the invite token) (§5.4) | — |
| `GET /auth/google/callback` | — (public) | Google's redirect target — exchanges the code for tokens, verifies the ID token, and either logs an existing user straight in (`sub`/email already on file) or redirects to the client's signup-completion/accept-invite screen carrying a short-lived signed **signup ticket** for a brand-new identity (§5.4) | — |
| `GET /auth/invite-preview?token=` | — (public) | Read-only lookup of the clinic profile a `super_user` already entered for a pending invite, so the accept-invite screen (§8.1) can pre-fill its form. Never exposes anything beyond what's needed to render that form (§5.4) | — |
| `POST /auth/accept-invite` | — (public, token in body) | Accept a provisioning invite — `token`, a chosen credential (`{ password }` or `{ googleTicket }`), and the (possibly-edited) clinic profile — sets the credential, applies the final clinic profile to the tenant, activates the user, clears the invite fields, and signs them in (§5.4) | — |
...
- **A new "Sign up" screen** (§8.1) for self-serve tenant creation —
  "Continue with Google" or "Use email and password," then the clinic
  profile form, all in one flow that ends signed in with a brand-new
  tenant.
- **A new "Accept invite" screen** (§8.1) for a freshly-provisioned admin's
  first login, reached only via the emailed invite link's token, not part
  of the normal login flow — the same Google-or-password choice the Sign
  up screen offers, plus the clinic profile the `super_user` already
  entered shown pre-filled and editable rather than typed from scratch.
...
- **`common/ScreenHeader`** is the one place the "title left, primary add-
  action right" header row gets rendered — `Tenants`, `Users`, `Patients`
  (§8.3), and `Appointments` (§8.10) all open with the exact same shape (a
  `screenHeading` `<h2>` in a `flex items-center justify-between` row next
  to a primary `<Button>`), so it was pulled into a shared component the
  same way `Screen`/`FormCard`/`Select` were, rather than staying
  copy-pasted a third and fourth time. `PatientList`/`AppointmentsScreen`
  own their own header this way now too — the "Add new patient"/"Add
  appointment" buttons moved from a full-width `<Button>` `App.tsx`
  rendered above each list into this row, matching the admin screens
  instead of being visually distinct from them.

### 8.1 Login, Sign up, and Accept invite (new — doesn't exist in the current MVP)
...
view of any patient data. A "Sign up" link off the login screen is the one
way into self-serve tenant creation, below.

**Sign up** (new, §5.4/§6): "Continue with Google" or "Use email and
password" first — establishing *who's* signing up — then the same clinic
profile form (name, mobile, clinic type mandatory; doctor name/
credentials/address/logo optional, §5.4/§5.6) `TenantForm` already
collects in this local-storage build. Submitting the password path calls
`POST /auth/signup` directly; the Google path only reaches this form after
the OAuth redirect returns with a signup ticket (§5.4), so the
clinic-profile step never re-asks for anything Google already verified.
Either way, submitting the completed form lands the new admin signed in
with their brand-new tenant.

**Accept invite** (new, §5.4/§6): the link in a freshly-provisioned
tenant's invite email opens the same Google-or-password choice as Sign up,
followed by the same clinic-profile form — except it arrives pre-filled
with whatever the `super_user` already typed at provisioning
(`GET /auth/invite-preview?token=`), editable rather than blank. On submit
(`POST /auth/accept-invite`, token from the URL, not typed in) the account
activates and the user lands signed in, same as a normal login. Not
reachable from the login screen itself; only via the emailed link.
...
- **Authentication**: password hashed with argon2id (or bcrypt if the
  ...
  storage in `sessions`, short expiry with sliding renewal. Google sign-in
  (§5.4) never touches a password at all — the ID token's signature/
  issuer/audience is verified server-side against Google's own keys, the
  OAuth client secret never reaches the browser, and the `state` param
  guarding the redirect is signed so it can't be forged into pointing a
  callback at someone else's pending invite.
...
- ~~Tenant provisioning is `super_user`-only in the real backend design,
  not self-serve signup~~ — **resolved: both are real, specified paths now
  (§4.1/§5.4/§6/§8.1)**. `POST /auth/signup` is a genuine public endpoint —
  self-serve was never actually blocked by anything architectural, only by
  this document not having decided yet. The `super_user`-provisioned
  invite path still exists alongside it, now sharing the same
  identity-choice (password/Google) and clinic-profile completion step
  self-serve signup uses, rather than being a separate set-password-only
  flow.
- **Account linking (password ↔ Google on one account) is not designed
  yet** (§5.4) — an account is stuck with whichever credential it signed
  up/accepted its invite with; there's no settings-screen action to add
  the other one later. The schema doesn't block it (`password_hash`/
  `google_sub` are already independently nullable on the same row), but
  the actual flow — re-authenticating with the existing method before
  allowing a new one to be attached, what happens if the Google email
  differs from the account's `users.email`, whether `PATCH /me` or a
  dedicated `POST /me/link-google` endpoint is the right shape — needs its
  own design pass before building it.
- **Whether a self-serve `POST /auth/signup` needs any anti-abuse
  gate is unspecified** (§5.4/§6) — email format is validated and
  `email_verified: true` is required for the Google path, but the
  password path has no email-confirmation step, CAPTCHA, or rate limit
  called out. Revisit if self-serve signup ever sees real-world abuse
  (throwaway clinics, spam registrations) rather than guessing at
  anti-abuse infrastructure now.
